...
downloadVerysync(){
    ...
    DOWNLOAD_LINK="http://dl.verysync.com/releases/${NEW_VER}/verysync-linux-${VDIS}-${NEW_VER}.tar.gz"
    curl ${PROXY} -L -H "Cache-Control: no-cache" -o ${ZIPFILE} ${DOWNLOAD_LINK}
    if [ $? != 0 ];then
       DOWNLOAD_LINK="http://dl-cn.verysync.com/releases/${NEW_VER}/verysync-linux-${VDIS}-${NEW_VER}.tar.gz"
       curl ${PROXY} -L -H "Cache-Control: no-cache" -o ${ZIPFILE} ${DOWNLOAD_LINK}
    fi
    if [ $? != 0 ];then
...
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        #NEW_VER=`curl ${PROXY} -k ${TAG_URL} --connect-timeout 10| grep 'tag_name' | cut -d\" -f4`
        TAG_URL="http://www.verysync.com/shell/latest"
        NEW_VER=`curl ${PROXY} -k ${TAG_URL} --connect-timeout 10`
